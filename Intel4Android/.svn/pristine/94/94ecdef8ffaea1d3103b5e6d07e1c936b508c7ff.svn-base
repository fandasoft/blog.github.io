...
public class OLTTestDetailGcActivity extends BaseActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d("zttjiangqq","taskIdOrId:"+taskIdOrId);
        initView();
        initValidata();
        initListener();
        bindData();
    ...
    public void initValidata() {
        ...
        for(int i = 0  ; i < tag_name_array.length ; i++){
            if(i == 0){
                fragments.add(EngineerInfoGcFragment.newInstance(taskIdOrId));
            }else if(i == 1){
                fragments.add(OLTListGcFragment.newInstance(taskIdOrId));
            }else {
                fragments.add(ApprovalHistoryGcFragment.newInstance(taskIdOrId));
            }
        }
