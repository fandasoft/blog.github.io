...
public class OkhttpManager {
    ...
    private static final String FILE_PREFIX="GAGASOFT_";
    //OKhttp对象实例
     ...
     * @param callBack
     */
    private void p_getAsyncParams(String url,Map<String,String> params, final DataCallBack callBack){
        ...
        if (params != null) {
            ...
            for (Map.Entry<String, String> entry : params.entrySet()) {
                ...
                sb.append('=');
                try {
                    Log.d("statistics", "get投递的key:" + key);
                    Log.d("statistics", "get投递编码前value:" + value);
                    value = URLEncoder.encode(value, "UTF-8");
                    Log.d("statistics", "get投递编码后value:" + value);
                    sb.append(value);
                    ...
                    if (response.isSuccessful()){
                        ...
                        for (String str:cookies){
                            if (str.startsWith("JSESSIONID")){
                                //将sessionId保存到本地
                                SharedPreferencesHelper.getInstance(IntelApplication.getInstance()).putStringValue(SharedPreferencesTag.KEY_SESSIONGID,str.split(";")[0]);
                            }
                        }
                    }
                    deliverSuccess(response.body().string(), callBack);
     ...
     * @param callBack
     */
    public static void getAsyncParams(String url,Map<String,String> params,DataCallBack callBack){
        getInstance().p_getAsyncParams(url, params, callBack);
    }
    /**
     * 进行POST异步请求数据
     * @param url
